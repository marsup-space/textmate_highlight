...
class _MultilineMatcher extends GrammarMatcher {
  ...
  /// Skip the remainder of the current line including the line break.
  ///
  /// When the input ends without a trailing newline, `.*\r?\n` would fail
  /// because `\r?\n` is mandatory. Post-fixing `$` as an alternative lets the
  /// match succeed at end-of-string so the while-loop body can advance past
  /// the final line — otherwise a `while`-anchored comment (Dart `///`) whose
  /// last continue-line has no trailing newline never produces the trailing
  /// line's scope, and its prose falls through to the top-level grammar.
  static final RegExp _skipLineRegExp = RegExp(r'.*(?:\r?\n|$)');
